...
#define MAX_RIGHT_LEFT_PRESSURE 1000 // (20PSI  ADC value -> (2.9V*4095)/3.3V)
#define MIN_TOP_PRESSURE 1100 // (10PSI  ADC value -> (0.9V*4095)/3.3V)
#define MIN_RIGHT_LEFT_PRESSURE 700 // (10PSI  ADC value -> (1.8V*4095)/3.3V)

WiFiUDP Udp;
const char *ssid_Router     = "Wifi_Name"; //TO BE REPLACED: network name
const char *password_Router = "Wifi_password"; //TO BE REPLACES: network password
...
void setupPWM() {
  ...
  ledcAttachPin(PUMP2, RIGHT_CHANNEL);
  ledcAttachPin(PUMP1, LEFT_CHANNEL); 
...
void loop() {
  ...
  if (currentSwitchState != previousSwitchState) {
    if (currentSwitchState == HIGH) {
      // Switch is activated, turn off the LEDs
    ...
    if(inflateTop || inflateRight || inflateLeft) {
      ...
      } else if(checkMinPressureTop && (ADCvalTop < MIN_TOP_PRESSURE)) {
        ledcWrite(TOP_CHANNEL, 0);
        inflateTop = true;
        checkMinPressureTop = false;
      }
      ...
      } else if(checkMinPressureLeft && (ADCvalLeft < MIN_RIGHT_LEFT_PRESSURE)) {
        ledcWrite(LEFT_CHANNEL, 0);
        inflateLeft = true;
        checkMinPressureLeft = false;
      }
      ...
      } else if(checkMinPressureRight && (ADCvalRight < MIN_RIGHT_LEFT_PRESSURE)) {
        ledcWrite(RIGHT_CHANNEL, 0);
        inflateRight = true;
        checkMinPressureRight = false;
      }
      ...
      if (digitalRead(PIN_BUTTON) == LOW) {
        ...
        if(checkMinPressureTop) {
          ADCvalTop = analogRead(PRESSURE_SENSOR_TOP);
          if(ADCvalTop < MIN_TOP_PRESSURE) {
            ledcWrite(TOP_CHANNEL, 0);
            inflateTop = true;
            checkMinPressureTop = false;
          } 
        ...
        if(checkMinPressureLeft) {
          ADCvalLeft = analogRead(PRESSURE_SENSOR_LEFT);
          if (ADCvalLeft < MIN_RIGHT_LEFT_PRESSURE) {
            ledcWrite(LEFT_CHANNEL, 0);
            inflateLeft = true;
            checkMinPressureLeft = false;
          }
        ...
        if(checkMinPressureRight) {
          ADCvalRight = analogRead(PRESSURE_SENSOR_RIGHT);
          if(ADCvalRight < MIN_RIGHT_LEFT_PRESSURE) {
            ledcWrite(RIGHT_CHANNEL, 0);
            inflateRight = true;
            checkMinPressureRight = false;
          }
